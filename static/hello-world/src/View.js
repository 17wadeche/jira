...
function PeopleFilter({ assignees, labels, selectedAssignees, displayMode, onChange, onDisplayModeChange }) {
  ...
  const options = [...labels.map((label) => ({ value: `label:${label}`, label, isGroup: true })), ...assignees.map((assignee) => ({ value: assignee, label: assignee, isGroup: false }))];
