...
function Chart({ series, config, forecast, personSeries = [] }) {
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} className="chart" role="img" aria-label="Burndown chart">
    ...
    {separatePeople && personSeries.map((person, personIndex) => <g key={person.assignee} style={{color:personColors[personIndex % personColors.length]}}><path d={path(person.series, 'remaining')} className="line personLine" style={{stroke:'currentColor'}}/>{config.showForecast !== false && (person.forecast || []).filter((scenario) => activeScenarioKeys.has(scenario.key)).map((scenario) => { const points=makeForecast(person.series,scenario.velocity); return <path key={scenario.key} d={`M ${x(person.series.length-1)} ${y(person.series[person.series.length-1]?.remaining || 0)} ${path(points,'remaining',person.series.length).replace('M','L')}`} className={`line personForecastLine ${scenario.key}Line`} style={{stroke:'currentColor'}}/>; })}</g>)}
