...
function View() {
  ...
  const kpis = [['Completed', `${metrics.completedPercent}%`, ''], ['Remaining', metrics.remainingWork, 'Work open'], ['This interval', metrics.activeInterval, 'Completed recently'], ['Total work', metrics.totalWork, 'Current scope'], ['Scope change', metrics.scopeChange, `${metrics.scopeChange >= 0 ? 'Added' : 'Removed'} net work`]];
  ...
  return <main className="page"><header className="topBar"><div><span className="eyebrow">Delivery analytics</span><h1>{chartTitle}</h1><span className="subtitle">TWD complaint handling burndown{lastUpdated && ` · Last refreshed ${lastUpdated.toLocaleString()}`}</span></div><div className="headerActions"><PeopleFilter assignees={assignees} labels={groupLabels} selectedAssignees={selectedAssignees} displayMode={config.peopleDisplay} teamLabel={teamLabel} onChange={(nextAssignees)=>loadData({...config,assignee:'all',assignees:nextAssignees})} onDisplayModeChange={(peopleDisplay)=>setConfig({...config,peopleDisplay})}/><button className="secondaryButton" onClick={()=>setSettings(!settings)}>⚙ Settings</button><button className="primaryButton" onClick={()=>loadData()}>↻ Refresh</button></div></header>
    ...
    <section className="insightHero" aria-label="Burndown summary"><div className="progressOrb" style={{'--progress': `${Math.min(100, Math.max(0, metrics.completedPercent || 0))}%`}}><span>{metrics.completedPercent}%</span><small>complete</small></div><div><span className={`healthBadge ${healthTone}`}>{healthTone === 'success' ? 'On track' : healthTone === 'warning' ? 'Needs focus' : 'At risk'}</span><h2>{metrics.remainingWork} items remain across {metrics.totalWork} total work items</h2><p>{scopeTone}: {Math.abs(metrics.scopeChange)} net item{Math.abs(metrics.scopeChange) === 1 ? '' : 's'} since the first interval. Latest interval closed {metrics.activeInterval} item{metrics.activeInterval === 1 ? '' : 's'}.</p></div></section>
    <div className="metricGrid">{kpis.map(([label,value,helper])=><div className={`metricCard ${label==='Scope change'?'scopeCard':''} ${label==='Scope change' ? (metrics.scopeChange>0?'scopeAdded':metrics.scopeChange<0?'scopeRemoved':'') : ''}`} key={label} title={label==='Scope change'?'Scope change is the net change in total work from the first interval to the latest interval. Positive means work was added; negative means work was removed.':undefined}><span>{label}{label==='Scope change'&&<i className="infoIcon" aria-label="Scope change is the net change in total work from the first interval to the latest interval">?</i>}</span><b>{value}</b>{helper&&<small>{helper}</small>}</div>)}</div>
